...
public class PicSlurperUtils {
	 ...
	 * @param statsFile
	 * @param statusConsumption
	 * @param forgetImageURLs
	 *            whether the actual image urls collected should be saved. On
	 *            the global stats file this should be true
	 * @throws IOException
	 */
	public static synchronized void updateStats(File statsFile, StatusConsumption statusConsumption,
			boolean forgetImageURLs) throws IOException
	{
	...
	public static synchronized void updateTweets(File outRoot, Status status) throws IOException {
		if (status == null)
			return;
		final File outFile = new File(outRoot, TWEET_FILE_NAME);
		final FileOutputStream fstream = new FileOutputStream(outFile, true);
		final PrintWriter pwriter = new PrintWriter(new BufferedWriter(new OutputStreamWriter(fstream, "UTF-8")));
