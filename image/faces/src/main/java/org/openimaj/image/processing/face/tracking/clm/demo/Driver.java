...
public class Driver extends KeyAdapter implements VideoDisplayListener<MBFImage> {
	...
	public void beforeUpdate(MBFImage frame) {
		...
		// MBFImage frame = ImageUtilities.readMBF(new
		// File("/Users/jsh2/Desktop/face.png"));
	...
	void Draw(MBFImage image, Matrix shape, int[][] con, int[][] tri, Matrix visi, float scale) {
		...
		// continue;
		//
		// p1 = new Point2dImpl((float)shape.get(tri[i][0],0),
		// (float)shape.get(tri[i][0]+n,0));
		// p2 = new Point2dImpl((float)shape.get(tri[i][1],0),
		// (float)shape.get(tri[i][1]+n,0));
		// image.drawLine(p1, p2, c);
		//
		// p1 = new Point2dImpl((float)shape.get(tri[i][0],0),
		// (float)shape.get(tri[i][0]+n,0));
		// p2 = new Point2dImpl((float)shape.get(tri[i][2],0),
		// (float)shape.get(tri[i][2]+n,0));
		// image.drawLine(p1, p2, c);
		//
		// p1 = new Point2dImpl((float)shape.get(tri[i][2],0),
		// (float)shape.get(tri[i][2]+n,0));
		// p2 = new Point2dImpl((float)shape.get(tri[i][1],0),
		// (float)shape.get(tri[i][1]+n,0));
		// image.drawLine(p1, p2, c);
		...
		for (int i = 0; i < con[0].length; i++) {
			if (visi.get(con[0][i], 0) == 0 || visi.get(con[1][i], 0) == 0)
				continue;
