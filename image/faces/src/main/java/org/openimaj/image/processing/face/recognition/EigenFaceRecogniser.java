...
/**
 * Implementation of a {@link FaceRecogniser} based on Eigenfaces. Any kind
		...
		LazyFaceRecogniser<FACE, Extractor<FACE>, PERSON>
{
	 ...
	 * @param internalRecogniser the face recogniser
	 */
	public EigenFaceRecogniser(Extractor<FACE> extractor, FaceRecogniser<FACE, ? extends FeatureExtractor<?, FACE>, PERSON> internalRecogniser) {
	 ...
	 * @param annotator the annotator
	 */
	public EigenFaceRecogniser(Extractor<FACE> extractor, IncrementalAnnotator<FACE, PERSON, ? extends FeatureExtractor<?, FACE>> annotator) {
	...
	EigenFaceRecogniser<FACE, PERSON> create(int numComponents, FaceAligner<FACE> aligner, int k, DoubleFVComparison compar, float threshold) 
{
