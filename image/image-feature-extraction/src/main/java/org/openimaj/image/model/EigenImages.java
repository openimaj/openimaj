...
/**
 * Implementation of EigenImages. Can be used for things like face
 * recognition ala the classic EigenFaces algorithm.
 * <p>
 * Fundamentally, the EigenImages technique is a way to perform
 * dimensionality reduction on an image using PCA. This implementation
 * can be trained through the {@link BatchTrainer} interface (which
 * will internally normalise the data and perform PCA). Once trained,
 * instances can be used as {@link FeatureExtractor}s to extract low(er)
 * dimensional features from images.
 * <p>
 * Methods are also provided to reconstruct an image from its feature
 * vector (see {@link #reconstruct(DoubleFV) and #reconstruct(double[])},
 * and to visualise a specific principal component as an image
 * (see {@link #visualisePC(int)}.
 * 
...
public class EigenImages implements BatchTrainer<FImage>, FeatureExtractor<DoubleFV, FImage>, ReadWriteableBinary {
	 ...
	 * 
	 * @param numComponents the number of PCs
	 */
