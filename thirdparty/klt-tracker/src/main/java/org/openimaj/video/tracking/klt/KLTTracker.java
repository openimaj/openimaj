...
public class KLTTracker {
			...
			boolean overwriteAllFeatures)
	{
		...
		while (true) {
	       ...
	       of the featurelist with -1's */
			...
			if (indx >= featurelist.features.length)  break;
	...
	private void _selectGoodFeatures(
		...
		int [][] pointlist;
		int npoints = 0;
		boolean overwriteAllFeatures = (mode == SelectionMode.SELECTING_ALL) ? true : false;
	     ...
	     of the two eigenvalues of the Z matrix */
		{
			...
			for (y = bordery ; y < nrows - bordery ; y += tc.nSkippedPixels + 1)
				for (x = borderx ; x < ncols - borderx ; x += tc.nSkippedPixels + 1)  {
					...
					if (val > limit)  {
						System.err.format("(_KLTSelectGoodFeatures) minimum eigenvalue %f is greater than the capacity of an int; setting to maximum value", val);
						val = limit;
					}
		...
		/* Check tc.mindist */
		if (tc.mindist < 0)  {
			System.err.format("(_KLTSelectGoodFeatures) Tracking context field tc.mindist is negative (%d); setting to zero", tc.mindist);
			tc.mindist = 0;
		}
	...
	public void selectGoodFeatures(FImage img) {
		...
		if (KLT_verbose >= 1)  {
			System.err.format("(KLT) Selecting the %d best features from a %d by %d image...  ", featurelist.features.length, img.width, img.height);
		}
	...
	public void replaceLostFeatures(FImage img)
	{
		...
		int nLostFeatures = featurelist.features.length - featurelist.countRemainingFeatures();
	...
	private int _trackFeature(
			...
			boolean lighting_insensitive)  /* whether to normalize for gain and bias */
	{
		...
		do  {
			...
			if (lighting_insensitive) {
				...
				_computeGradientSumLightingInsensitive(gradx1, grady1, gradx2, grady2, img1, img2, x1, y1, xy2[0], xy2[1], width, height, gradx, grady);
			} else {
	...
	//	      fname = String.format("./debug/kltimg_trans_diff_win%03d.%03d.pgm", glob_index, counter);
	//	      printf("%s\n", fname);
	...
	//	      aff_diff_win.data = imgdiff;
	//	      fname = String.format("./debug/kltimg_aff_diff_win%03d.%03d_1.pgm", glob_index, counter);
	//	      printf("%s\n", fname);
	//	      _KLTWriteAbsFloatImageToPGM(aff_diff_win, fname,256.0);
	//	      
	//	      _am_computeAffineMappedImage(img2, *x2, *y2,  *Axx, *Ayx , *Axy, *Ayy, width, height, imgdiff);
	//	      aff_diff_win.data = imgdiff;
	//	      fname = String.format("./debug/kltimg_aff_diff_win%03d.%03d_2.pgm", glob_index, counter);
	//	      printf("%s\n", fname);
	//	      _KLTWriteAbsFloatImageToPGM(aff_diff_win, fname,256.0);
	//	#endif
	//	      
	...
	//	#ifdef DEBUG_AFFINE_MAPPING    
	//	      aff_diff_win.data = imgdiff;
	//	      fname = String.format("./debug/kltimg_aff_diff_win%03d.%03d_3.pgm", glob_index,counter);
	//	      printf("%s\n", fname);
	//	      _KLTWriteAbsFloatImageToPGM(aff_diff_win, fname,256.0);
	//	      
	...
	public void trackFeatures(FImage img1, FImage img2)
	{
		...
		if (KLT_verbose >= 1)  {
			System.out.println(String.format("(KLT) Tracking %d features in a %d by %d image...  ", featurelist.countRemainingFeatures(), ncols, nrows));
		}
			...
			if (featurelist.features[indx].val >= 0)  {
				...
				if (val == KLT_OOB) {
					...
					if (tc.affineConsistencyCheck >= 0 && val == KLT_TRACKED)  { /*for affine mapping*/
						...
						//								featurelist.features[indx].aff_img = new FImage((tc.affine_window_height+border), (tc.affine_window_width+border));
						//								featurelist.features[indx].aff_img_gradx = new FImage((tc.affine_window_height+border), (tc.affine_window_width+border));
						//								featurelist.features[indx].aff_img_grady = new FImage((tc.affine_window_height+border), (tc.affine_window_width+border));
						...
						//								featurelist.features[indx].aff_x = xloc - (int) xloc + (tc.affine_window_width+border)/2;
						//								featurelist.features[indx].aff_y = yloc - (int) yloc + (tc.affine_window_height+border)/2;;
						//							}else{
						//								/* affine tracking */
						//								val = _am_trackFeatureAffine(featurelist.features[indx].aff_x, featurelist.features[indx].aff_y,
						//									&xlocout, &ylocout,
