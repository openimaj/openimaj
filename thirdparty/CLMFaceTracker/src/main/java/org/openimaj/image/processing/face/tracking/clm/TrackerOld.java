...
/**
 *	The initial ported version of the CLMTracker that can only track
 *	a single face in an image. It's had only a few small changes to allow it
 *	to work with the list which the face detector class now returns.
 *	Unless you're sure you only want to track a single face, you should probably
 *	use the new {@link Tracker} class.
 */
public class TrackerOld {
	...
	TrackerOld(CLM clm, FDet fdet, MFCheck fcheck, Matrix rshape, double[] simil)
	{
		_clm = clm; 
		_fdet = fdet; 
		_fcheck = fcheck;
	...
	//===========================================================================
	static TrackerOld Load(final String fname) throws FileNotFoundException
	{
	 ...
	 *	@return a tracker
	 */
	public static TrackerOld Load(final InputStream in)
	{
		BufferedReader br = null;
		try {
	...
	//===========================================================================
	static TrackerOld Read(Scanner s, boolean readType)
	{
		if (readType) { 
			int type = s.nextInt();
			assert(type == IO.Types.TRACKER.ordinal());
		}
		TrackerOld tracker = new TrackerOld();
