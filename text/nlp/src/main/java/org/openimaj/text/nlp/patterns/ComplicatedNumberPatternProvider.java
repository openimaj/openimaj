...
//	String Number = "(?:\\b|[$])\\d+" + pos_lookahead(regex_or("[^,\\d]","$"));
	...
	String NumNum = String.format("(?:\\b|[$])\\d+%s",regex_or(number_ends))+ pos_lookahead(regex_or(after_number));
	String NumberWithCommas = "(?:\\b|[$])(?:\\d+,)+\\d{3}(?:.\\d+)?"+ pos_lookahead(regex_or(after_number));
	
	@Override
	public String patternString() {
		return regex_or(NumNum,NumberWithCommas);
	}
}
